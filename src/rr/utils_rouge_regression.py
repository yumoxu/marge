...
class RrSentenceProcessor(DataProcessor):
    ...
    def preprocess_json(self, json_obj, rouge_c, metric, no_query):
        ...
        if no_query:
            ...
            if type(json_obj['masked_summary']) is str:
                text_a = json_obj['masked_summary']
            else:  # masked_summary is a word list
                assert type(json_obj['masked_summary']) is list
                text_a = ' '.join(json_obj['masked_summary'])  
