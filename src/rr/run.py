...
from rr.utils_rouge_regression import (compute_metrics, convert_examples_to_features, processors)
from rr.checkpoint_utils import (update_checkpoint_dict, clean_outdated_checkpoints)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    corr = 100 * results['corr']
                    record = f'{global_step}\t{avg_loss:.8f}\t{eval_loss:.8f}\t{corr:.4f}\n'
...
def get_args():
    ...
    parser.add_argument('--with_sub', action='store_true',
                        help="Whether to append an extra token (set to [unused1])for subqueries/sentence in the masked summary.")
...
def main():
    ...
    tokenizer = BertTokenizer.from_pretrained(args.tokenizer_name if args.tokenizer_name else args.model_name_or_path,
        ...
        additional_special_tokens=['[SLOT]', '[SUBQUERY]'])  # in case slot is represented with [SLOT] (compared to [MASK])

    # debug special tokens
    # print('[SUBQUERY]: {}'.format(tokenizer.convert_tokens_to_ids('[SUBQUERY]')))
    # print('[SUBQUERY-RANDOM]: {}'.format(tokenizer.convert_tokens_to_ids('[SUBQUERY-RANDOM]')))
